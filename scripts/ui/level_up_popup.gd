...
const UPGRADE_LABELS := {
	...
	"chain": "+1 Chain (hit 1 more nearby enemy)",
...
func _eligible_upgrade_ids() -> Array[String]:
	...
	if skill != null:
		...
		# (2026-07-24) "+1 Chain" is dead in the same two ways as "+1 Arrow":
		# a trap has no projectile to spread FROM, and past MAX_CHAIN_COUNT the
		# clamp in effective_chain_count() eats the extra. Asked for explicitly
		...
		if ignores_count or player.effective_chain_count(skill) >= player.MAX_CHAIN_COUNT:
			ids.erase("chain")
