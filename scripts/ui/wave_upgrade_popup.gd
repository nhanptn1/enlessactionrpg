...
# (2026-07-23) Was ONE greyscale base recoloured per element with a
# StyleBoxTexture multiply-tint. Multiply can only darken toward a single hue,
# which is why Lightning could never get its authored purple+yellow TWO-TONE
# (entry 69 flagged it as needing custom art -- it didn't). These are duotone
# remaps of that same base, generated by scripts/tools/gen_card_frames.gd:
# shadows take a dark tone, highlights a light one, so Lightning reads purple
# in the recesses and yellow on the raised edges. Every other line keeps its
# approved hue and simply gains depth instead of being one flat wash. Applied
# untinted (modulate stays white) -- the colour lives in the texture now.
const CARD_FRAMES := {
	UpgradeResource.ElementType.PHYSICAL: "res://art/ui/card_frame_duo_physical.png",
	UpgradeResource.ElementType.FIRE: "res://art/ui/card_frame_duo_fire.png",
...
func _ready() -> void:
	...
	for element in CARD_FRAMES:
		var stylebox := StyleBoxTexture.new()
		stylebox.texture = load(CARD_FRAMES[element])
